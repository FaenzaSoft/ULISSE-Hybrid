// CONSUMO ZERO HYBRID - Ver. 2.1
// modifiche: aggiunte 2 linee di codice per evitare che la centralina rimanga in standby anche di giorno con batterie scariche.
//
...
//
//
// AVVERTENZA: per evitare problemi di surriscaldamento delle resistenze (4 x 1 ohm 5 watt cadauna) è stato impostato il limitatore a 7 amper:
//
//
...
void loop()
{
    ...
    //            if (volt_batterie < 1150)        // con batteria da 12 volt:
  {
    ...
    analogWrite (caricoPin, 250);         // per consentire la carica appena il pannello ha tensionme:
    if (conta_stacco > 100)               // stacca con ritardo per evitare lo sbalzo di consumo hardware quando si riaccendono i faretti:
    {
